...
export function PrescriptionEvidenceWorkspace() {
  ...
  return <AppShell><section className="evidence-page"><header className="page-heading evidence-heading"><div><p className="eyebrow">Prescription evidence</p><h1>Review evidence before it can inform care.</h1><p>OCR output is advisory. It never creates a medication, regimen, reminder, or diagnosis automatically.</p></div><Link className="button button-secondary" href="/"><RefreshCw size={16} /> Overview</Link></header>{state.error && <p className="evidence-error" role="alert">{state.error}</p>}<section className="evidence-flow"><div className="flow-heading"><div><p className="eyebrow">Step 1</p><h2>Choose a profile and prescription</h2></div><span>Core-authoritative</span></div><div className="evidence-controls"><label>Profile<select value={profileId} onChange={(event) => void load(event.target.value, '')}><option value="">Select a profile</option>{state.profiles.map((profile) => <option value={profile.id} key={profile.id}>{profile.preferredName}</option>)}</select></label><label>Prescription<select value={prescriptionId} onChange={(event) => void load(profileId, event.target.value)} disabled={!profileId}><option value="">Select a prescription</option>{state.prescriptions.map((prescription) => <option value={prescription.id} key={prescription.id}>{prescription.prescriberLabel ?? `Prescription ${prescription.id.slice(0, 8)}`}</option>)}</select></label><button className="button button-secondary" disabled={!profileId || state.loading} onClick={() => void createPrescription()} type="button">New prescription</button></div></section><section className="evidence-flow upload-flow"><div className="flow-heading"><div><p className="eyebrow">Step 2</p><h2>Upload prescription evidence</h2><p>JPEG, PNG, WebP, or PDF up to 10 MB. Evidence remains profile-scoped in Core.</p></div></div><div className="upload-row"><span className="upload-icon"><FileUp size={22} /></span><div><strong>Choose a prescription file</strong><p>The upload is associated with the selected profile and prescription.</p></div><label className="button button-primary">Choose file<input aria-label="Choose prescription evidence" type="file" accept="image/jpeg,image/png,image/webp,application/pdf" disabled={!prescriptionId || state.loading} onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadEvidence(file); }} /></label></div></section><section className="evidence-results"><article className="result-card"><p className="eyebrow">Evidence status</p>{state.loading ? <p className="result-placeholder">Loading Core-authoritative evidence…</p> : state.evidence.length === 0 ? <p className="result-placeholder">No evidence is attached to this prescription yet.</p> : <ul className="evidence-list">{state.evidence.map((item) => <li key={item.id}><strong>{item.status.replaceAll('_', ' ')}</strong><span>{item.contentType} · {(item.declaredSizeBytes / 1024).toFixed(0)} KB</span></li>)}</ul>}</article><article className="result-card"><p className="eyebrow">OCR review status</p>{state.extractions.length === 0 ? <p className="result-placeholder">No extraction candidate is ready for human review.</p> : <ul className="evidence-list">{state.extractions.map((item) => <li key={item.id}><strong>{item.status.replaceAll('_', ' ')}</strong><span>{item.resultSource === 'demo' ? `Demo provenance: ${item.demoFixtureId ?? 'marked fixture'}` : 'ML provenance pending operator policy'}</span></li>)}</ul>}<p className="review-note"><ShieldCheck size={16} /> Human confirmation remains mandatory; clinical changes stay outside this screen.</p></article></section></section></AppShell>;
}
